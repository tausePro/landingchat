/**
 * Página de detalle de orden para dominios personalizados
 * Obtiene la organización a partir del custom_domain en el header host
 */
...
export default async function OrderTrackingPage({ params, searchParams }: OrderPageProps) {
    ...
    if (order.payment_method === "epayco" && order.payment_status === "pending") {
        ...
        if (reconciliation.reconciled) {
            const refreshed = await getOrderDetails(publicOrganization.slug, orderId, access)
            if (refreshed) {
                        ...
                        <div className="md:col-span-2 space-y-8">
                            <div>
                                ...
                                <div className="space-y-4">
                                    {order.items.map((item: { quantity: number; product_name: string; variant_info?: unknown; total_price: number }, i: number) => (
