...
export default async function ProductDetailPage({ params }: ProductDetailPageProps) {
    ...
    return (
        <div className="relative flex h-auto min-h-screen w-full flex-col overflow-x-hidden font-display bg-background-light dark:bg-background-dark text-slate-800 dark:text-slate-200">
            {/* Header */}
            <header className="flex items-center justify-between whitespace-nowrap border-b border-solid border-slate-200 dark:border-slate-800 px-10 py-3 bg-white dark:bg-background-dark/50 sticky top-0 z-50 backdrop-blur-sm">
                <div className="flex items-center gap-8">
                    <Link href={`/store/${slug}`} className="flex items-center gap-4 text-slate-900 dark:text-white">
                        {organization.logo_url ? (
                            <Image src={organization.logo_url} alt={organization.name} width={24} height={24} className="size-6" />
                        ) : (
                        ...
                        <h2 className="text-lg font-bold tracking-[-0.015em]">{organization.name}</h2>
                    </Link>
                </div>
                <div className="flex flex-1 justify-end gap-8">
                    <div className="flex items-center gap-9">
                        <Link href={`/store/${slug}`} className="text-sm font-medium text-slate-900 dark:text-slate-50">Inicio</Link>
                        <Link href={`/store/${slug}/products`} className="text-sm font-medium text-primary">Tienda</Link>
                    </div>
                    <div className="flex gap-2">
                        <button className="flex max-w-[480px] cursor-pointer items-center justify-center overflow-hidden rounded-lg h-10 bg-slate-100 dark:bg-slate-800 text-slate-900 dark:text-slate-50 gap-2 text-sm font-bold leading-normal tracking-[0.015em] min-w-0 px-2.5">
