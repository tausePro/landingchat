...
import { approveStoreCommission, markStoreCommissionPaid, type StoreCommission } from "./actions"
...
export function StoreCommissionsTable({ initial }: { initial: StoreCommission[] }) {
    const [rows, setRows] = useState<StoreCommission[]>(initial)
    ...
    if (rows.length === 0) {
        return <p className="text-sm text-muted-foreground">Aún no hay comisiones. Se generan cuando un cliente referido compra.</p>
    }
            ...
            <table className="w-full text-sm">
                <thead className="bg-slate-50 dark:bg-slate-900 text-left text-xs uppercase tracking-wider text-muted-foreground">
                    <tr>
                        ...
                        <th className="px-4 py-3">Base (productos)</th>
                ...
                <tbody>
                    {rows.map((r) => (
                        <tr key={r.id} className="border-t border-slate-100 dark:border-slate-800">
                            <td className="px-4 py-3 text-slate-700 dark:text-slate-200">{r.affiliateName}</td>
                            ...
                            <td className="px-4 py-3 text-right">
                                {r.status === "pending" && (
                                    <Button size="sm" disabled={busy === r.id} onClick={() => act(r.id, approveStoreCommission, "approved")}>
                                        Aprobar
                                    </Button>
                                )}
                                {r.status === "approved" && (
                                    <Button size="sm" disabled={busy === r.id} onClick={() => act(r.id, markStoreCommissionPaid, "paid")}>
                                        Marcar pagada
                                    </Button>
                                )}
