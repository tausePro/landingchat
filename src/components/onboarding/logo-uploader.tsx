...
export function LogoUploader({ organizationId, onUploadComplete }: LogoUploaderProps) {
    ...
    const uploadLogo = useCallback(async (file: File) => {
        ...
        if (!validTypes.includes(file.type)) {
            setError('Formato no válido. Usa PNG, JPG o SVG.')
            return
        }
    ...
    }, [organizationId, onUploadComplete, supabase])
        ...
        <div className="flex flex-col gap-2">
            ...
            {preview ? (
                <div className="flex flex-col items-center gap-4 rounded-xl border-2 border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 p-8">
                    <img src={preview} alt="Logo preview" className="h-24 w-24 object-contain rounded-lg" />
                    <button
                ...
                >
                    ...
                    <div className="text-center">
                        <p className="text-sm font-semibold text-slate-800 dark:text-slate-200">
                            {uploading ? 'Subiendo...' : 'Arrastra y suelta tu logo aquí'}
                        </p>
                        <p className="text-xs text-slate-500 dark:text-slate-400">
                            Recomendado: 200x200px, PNG, JPG, SVG (máx. 2MB)
                        </p>
                    </div>
