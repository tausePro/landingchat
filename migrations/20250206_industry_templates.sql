...
-- 2. Agregar columna enabled_modules a organizations
ALTER TABLE organizations
    ADD COLUMN IF NOT EXISTS industry_slug TEXT,
    ADD COLUMN IF NOT EXISTS enabled_modules TEXT[] DEFAULT '{}';

-- 3. Crear índice para búsqueda por industry_slug
CREATE INDEX IF NOT EXISTS idx_organizations_industry_slug
    ON organizations(industry_slug);

-- 4. Foreign key opcional (no estricta para flexibilidad)
-- No agregamos FK real porque queremos permitir industrias custom
...
COMMENT ON COLUMN organizations.industry_slug IS 'Slug de la industria seleccionada en onboarding';
COMMENT ON COLUMN organizations.enabled_modules IS 'Array de slugs de módulos activos para esta organización';

-- 9. RLS para industry_templates (solo lectura pública)
...
-- 10. RLS para module_definitions (solo lectura pública)
